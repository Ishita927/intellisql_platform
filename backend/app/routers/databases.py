...
#     # NEW: build embeddings for RAG-based retrieval
#     await crud.store_schema_embeddings(
#         tenant_id=current_user.tenant_id,
#         database_connection_id=database_id,
#         tables_data=tables_data,
#     )
...
from ..schemas import (
    DatabaseTestRequest, DatabaseTestResponse,
    DatabaseConnectionCreate, DatabaseConnectionRead,
    SchemaScanResponse, TableSchema, ColumnInfo, ForeignKeyInfo,
)
from ..models import User
from ..security import decrypt_value
from .. import crud
...
def _connect_args_for_host(host: str) -> dict:
    """Local Postgres doesn't support SSL; cloud-hosted Postgres (Neon, etc.) requires it."""
...
async def test_connection(
    ...
    test_engine = create_async_engine(
        test_url, connect_args=_connect_args_for_host(payload.host)
    )
    try:
        async with test_engine.connect() as conn:
            await conn.execute(text("SELECT 1"))
...
async def scan_schema(
    ...
    company_engine = create_async_engine(
        company_db_url, connect_args=_connect_args_for_host(connection.host)
    )
    try:
        async with company_engine.connect() as conn:
            tables_data = await conn.run_sync(_reflect_schema)
